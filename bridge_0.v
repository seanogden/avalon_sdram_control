...
input		[(PRM_DATA_WIDTH-1):0]	avalon_readdata;

input		[(PRM_ADDR_WIDTH-1):0]	address;
input		[(PRM_BYTE_ENABLE-1):0]	byte_enable;
input								write;
input								read;
input		[(PRM_DATA_WIDTH-1):0]	write_data;

// Bidirectionals

// Outputs
output		[(PRM_ADDR_WIDTH-1):0]	avalon_address;
output		[(PRM_BYTE_ENABLE-1):0]	avalon_byteenable;
output								avalon_read;
output								avalon_write;
output		[(PRM_DATA_WIDTH-1):0]	avalon_writedata;

output		[(PRM_DATA_WIDTH-1):0]	read_data;
output								acknowledge;
//
//
